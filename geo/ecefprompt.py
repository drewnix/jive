...
for line in fileinput.input():
    mess = line.rstrip().lstrip().split(',')
    mess2 = [ [ float(p) for p in m.split() ] for m in mess ]

    angle = map(ecef, mess2[0:3])

    if orient(*angle) < 0:
        sys.stderr.write("Orientation is incorrect, len is %s, reversed polygon: \n" % len(mess))
        mess.reverse()
        print ",".join(mess)
    else:
        print "poly is fine.."
